...
int Peso_conocido = 162.5;// en gramos | MODIFIQUE PREVIAMENTE CON EL VALOR DE PESO CONOCIDO!!!
...
int Volumen_deseado = 220;// en mL | MODIFIQUE PREVIAMENTE CON EL VALOR DE VOLUMEN DESEADO!!!
...
int Var = 1022;
float pesoRecipiente = 162.7+3.2;
...
void loop() {
  ...
  if(Serial.available()>0){
    option=Serial.read();
    if(option=='t'){
      balanza.tare(20); //Tarar con el comando 't'
    }
    if(option=='m'){
            ////// Motor paso a paso con el comando 'm' //////
...
float MedidaBalanza(void)
{
  ...
  Serial.print(promPL-pesoRecipiente, 1);
  Serial.println(" g");
...
void DosificarB(float pesoRecipiente)
{
  ...
  while ((Volumen_deseado / Densidad)>= pesoLiquido)
  {
    ...
    if (pesoLiquido>=(Volumen_deseado / Densidad)*0.6) 
    {
    break;
    }
  }
...
while ((Volumen_deseado / Densidad)>= pesoLiquido)
  {
    Adelante = map(Var, 0, 1023, 0, 190);
    analogWrite(Input1, Adelante);
        
    pesoLiquido = MedidaBalanza() - pesoRecipiente;

    if (pesoLiquido>=(Volumen_deseado / Densidad)*0.75) 
    {
    break;
    }
  }
...
while ((Volumen_deseado / Densidad)>= pesoLiquido)
  {
    Adelante = map(Var, 0, 1023, 0, 56);
    analogWrite(Input1, Adelante);
        
    pesoLiquido = MedidaBalanza() - pesoRecipiente;

    if (pesoLiquido>=(Volumen_deseado / Densidad)*0.963) 
    {
    break;
    }
  }
  ...
  if ((Volumen_deseado / Densidad)*0.96<= pesoLiquido)
  {
    OnOff = 0;
...
void ControlMotorPasoaPaso(void)
{
  ...
  digitalWrite(DirPin, LOW); // Gira del lado que es >:v
}
