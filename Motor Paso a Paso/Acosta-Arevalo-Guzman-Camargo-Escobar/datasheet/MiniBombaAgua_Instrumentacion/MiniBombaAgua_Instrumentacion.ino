...
void CalibracionBalanza(void)
{
  ...
  balanza.set_scale(-959.86); ; // Ajusta la escala correspondiente
...
void DosificarB(float pesoRecipiente)
{
  ...
  while (((Volumen_deseado / Densidad)*0.85 >= pesoLiquido)&&(pesoLiquido >= (Volumen_deseado / Densidad)*0.5))
  {
    Adelante = map(Var, 0, 1023, 0, 160);
    analogWrite(Input1, Adelante);
    pesoLiquido = MedidaBalanza() - pesoRecipiente;
        
  }
  while (((Volumen_deseado / Densidad)*0.99 >= pesoLiquido)&&(pesoLiquido >= (Volumen_deseado / Densidad)*0.85))
  {
    Adelante = map(Var, 0, 1023, 0, 100);
    analogWrite(Input1, Adelante);
    pesoLiquido = MedidaBalanza() - pesoRecipiente;
  ...
  if ((Volumen_deseado / Densidad)*0.99 <= pesoLiquido)
  {
    OnOff = 0;
