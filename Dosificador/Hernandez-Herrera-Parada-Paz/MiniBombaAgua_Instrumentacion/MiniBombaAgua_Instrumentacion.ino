...
float Densidad = 0.998;// en g/mL | MODIFIQUE PREVIAMENTE CON EL VALOR DE DENSIDAD CORRESPONDIENTE!!!

int Volumen_deseado = 150;// en mL | MODIFIQUE PREVIAMENTE CON EL VALOR DE VOLUMEN DESEADO!!!
...
float pesoEnMedioSegundo=5.586*0.643*Densidad;
boolean seguir=true;  
float pesoDeseado=Volumen_deseado*Densidad;
int time2;
int time1;
...
void setup() {
  ...
  time1=millis();
  delay(1000);
  Serial.println("Se completo el llenado rápido");
}
...
void loop() {
 ...
 pesoActual=MedidaBalanza();
  Serial.print(pesoActual, 1);
   Serial.println(" g");
 if(seguir==true){
  Serial.println("Empezó a llenar lento");
  
  analogWrite(Input1, llenar_lento);
 if (pesoDeseado< pesoActual+pesoEnMedioSegundo)
 {

  float dif=pesoDeseado-pesoActual;
  Serial.println(pesoDeseado);
  Serial.println(pesoActual);
  Serial.println(dif);
  float difvol=dif/Densidad;
  Serial.println(difvol);
  int tiempoFinal=difvol/5.586;
  delay(tiempoFinal);
  Serial.println(tiempoFinal);
  analogWrite(Input1, Detener);
  Serial.println("Se detuvo el llenado lento");
  seguir=false;
 }
 }
 delay(500);
...
float llenado_rapido(void)
{
  while(0.6*Volumen_deseado>MedidaBalanza())
 {
//  float volumen_a_llenar=0.6*Volumen_deseado; // ml
//  float tiempo_de_llenado=volumen_a_llenar/9.461;
